...
glm::vec3 SceneObject::lighting(const glm::vec3& lightPos, const glm::vec3& viewVec, const glm::vec3& hit) {
	...
	glm::vec3 colorSum = diffuseTerm * color_ + specularTerm * glm::vec3(1);
	return colorSum;
}
...
void SceneObject::setTransform(const glm::mat4& trans) {
    transf = trans;
    inverseTransf = glm::inverse(trans);
    normalTransf = glm::transpose(inverseTransf);
}
